...
module.exports = {
    ...
    authorise: function( cb_success, cb_failure ) {
        ...
        /**
         * Used the auth token to authenticate
         * @param auth {Object} the successfully populated auth token
         */
...
//            });
        ...
        /**
         * Attempts to use oAuth to authenticate.
         * 1) If a token is specified in the config then that is used
         * 2) Attempt basic authentication (required to request a token - bails on fail)
         * 3) Builds oAuth token (not all parameters specified here are required)
         * 4) Attempts to create the auth token and use it to authenticate
         */
        var getOAuth = function() {
            auth.type = 'oauth';        // @todo this gets reset to basic during getBasicAuth() check
            ...
            if ( !getBasicAuth() ) {            // @todo only validates that username and password have been specified
            ...
            // Build oauth object to request auth token
            // Auth token is added to the object on success and passed to storeAuth() to authenticate with
            // @todo Not all of these parameters are required to request a token
            // @todo scopes in the config must be set closed up (no whitespace) - add utility function to strip whitespace
            auth.type = 'basic';            // This is included for code clarity - getBasicAuth() above will already have set this
            ...
            auth.note = octofish.config.auth.note;
            auth.note_url = octofish.config.auth.note_url;
