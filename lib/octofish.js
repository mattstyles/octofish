...
module.exports = {
    ...
    authorise: function() {
        ...
        var getOAuth = function() {
            ...
            // If a valid token has been passed then just return that
            ...
            // Perform basic authorisation and return null if failed
            var basicAuth = getBasicAuth();
            if ( !basicAuth ) {
                console.warn( 'Basic authentication failed --- can not proceed with oauth' );
                console.log( 'Basic auth object used to attempt basic authentication: ' );
                console.dir( basicAuth );
                return null;
            }
            ...
            auth.scopes = octofish.config.auth.scope ? octofish.config.auth.scope.split(',') : '';      // todo fix this - passes incorrectly unless specified in config
        ...
        var auth = getAuth( octofish.config.auth.type );

        // If authentication vars are valid then attempt to authenticate with github
        if ( auth ) {
            console.log( 'Authentication with github --- using ' + octofish.config.auth.type + ' authorisation' );
        } else {
            console.warn( 'Authentication failure --- Unauthorised access to github --- max rate limit of 60 reqs per hour' );
        }
    }
};
